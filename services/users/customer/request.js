...
const request = {
        ...
        .then(async (result) => {
          ...
          if (result.length > 0) {
            for (let i = 0; i < result.length; i++) {
              (function (i) {
                result[i].users_master.avg_rating = Number.parseFloat(
                  result[i].users_master.avg_rating
                ).toFixed(1);
  ...
  checkGroupCode: async (groupcodeinfo) => {
    ...
    if (groupcodeinfo.user_id) {
      user_id = groupcodeinfo.user_id;
    }
    var promises = [];
  ...
  updateStatusDuplicateRequest: async (data) => {
    ...
    if(data.payment_status=="done" && data.status === "confirm"){
		updatedData.status = "wip";
	}
	 if(data.payment_status=="pending" && data.status === "confirm"){
...
/**
   * update request duplicate request
   */
  updateReorderRequest: async (data) => {
    let updatedData = {
      status: data.status,
      promo_code:data.promo_code,
      price:data.price,
      discount_promo_code:data.discount_promo_code,
    };
    if(data.payment_status=="done"){
		updatedData.status = "wip";
		updatedData.payment_status = "done";
	}
  ...
  /*
   * check user have session or not for permission 
   * 
   */ 
  checkHaveRequest: async (user_id) => {
    var promises = [];
    let countUsedGroupSql =
      "SELECT count(*) AS total_session FROM requests WHERE customer_id = '" +
      user_id +
      "' && status IN ('pending','wip','completed')"; //
    ...
    promises.push(
      dbConnection
        ...
        .then(function (haveSessionCountResult) {
          haveSessionCount = haveSessionCountResult;
        }),
     
    );
    return Promise.all(promises)
      .then(function (result) {
        if(haveSessionCount[0].total_session>0){
			return true;
		}else{
			return false;
		}
  ...
  getGroupCodeIdByRequestId:async (request_id) => {
    var promises = [];
    let Sql =
      "SELECT requests.id AS requests_id,group_code.id AS group_code_id FROM requests LEFT JOIN group_code ON group_code.code=requests.group_code WHERE requests.id = '" +
      request_id +
      "' "; //
       ...
       return await dbConnection
        ...
        .then(async (SqlResult) => {
          ...
          if(SqlResult.length>0){
			return await SqlResult[0].group_code_id;
			}else{
			 return null;
			}
        }).catch(function (err) {
