...
requestMonthList = (req,res) => {
    ...
    let SelectSql = "SELECT users_master.about_me, users_master.first_name,users_master.last_name,users_master.profile_image,users_master.user_type,requests.id,requests.request_number,requests.therapy_type,requests.status,requests.apointment_date_time,requests.price FROM requests ";
    
  //  SelectSql += " LEFT JOIN users_master on users_master.id = requests.therapist_id WHERE requests.customer_id = :customer_id AND MONTH(apointment_date_time)=:month AND requests.status !='draft' AND requests.deleted_at is null";
  
  SelectSql += " LEFT JOIN users_master on users_master.id = requests.therapist_id WHERE requests.customer_id = :customer_id AND requests.status !='draft' AND requests.request_delete='no' AND requests.deleted_at is null";
    
    

     dbConnection.query(SelectSql, { 
            type: dbConnection.QueryTypes.SELECT,
            replacements: {customer_id:customer_id,month:data.month}
			...
			for(var j=0; j < result.length; j++){
				(function(j){
					...
					var dateTimeJson =  {
                        ...
                        'apointment_time':moment(result[j].apointment_date_time).format("hh:mm A"),
