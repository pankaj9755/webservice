...
videoAgreementAction = function(req, res) {
    ...
    dbConnection.query(requestsSql, { 
    	...
    	if(requestsData.length > 0){
			if(user_id!=requestsData[0].customer_id){
			response.msg = "Soory! You have no permission to perform this action!";
	        response.statusCode = 400;
	        res.statusCode = 400;
	        res.send(response);
		}
			var therapistTemplateSql = "SELECT * FROM email_template WHERE id = 14";
			dbConnection.query(therapistTemplateSql, {
				...
				if(ACTION=='accept'){
					var ACTIONED = 'accepted';
				}else{
					var ACTIONED = 'declined';
				}				
				...
				var therapistHtmlTemplate = therapistTemplateResult[0].body.replace(/[\s]/gi, ' ').replace('[ACTIONED]',ACTIONED).replace('[ACTION]',ACTION).replace('[THERAPIST_NAME]',THERAPISTNAME).replace('[CUSTOMER_NAME]',CUSTOMERNAME).replace('[LOGO]',LOGO).replace('[REQUEST_NUMBER]',REQUESTNUMBER).replace('[THARAPY_TYPE]',THARAPYTYPE).replace('[APPOINTMENT_DATE]',APPOINTMENTDATE).replace('[PATIENT_NAME]',CUSTOMERNAME); 
				
				adminEmail.push('wandile@syked.co.za');
				adminEmail.push('info@syked.co.za');
				var subject  = therapistTemplateResult[0].subject.replace(/[\s]/gi, ' ').replace('[ACTION]',ACTIONED);
...
module.exports = videoAgreementAction;
