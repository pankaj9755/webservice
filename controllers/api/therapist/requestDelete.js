...
requestDelete = (req,res) => {
    ...
    let SelectSql = "SELECT * From requests where therapist_id = '"+user_id+"' And id='"+data.request_id+"'";
    dbConnection.query(SelectSql, { 
        type: dbConnection.QueryTypes.SELECT,
    }).then(function(resultRequest) {
    	if(resultRequest.length > 0){

            let DeleteSql = "UPDATE requests set request_therapist_delete='yes' where therapist_id = '"+user_id+"' And id='"+data.request_id+"'";
