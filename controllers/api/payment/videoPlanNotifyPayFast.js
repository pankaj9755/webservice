...
videoPlanNotifyPayFast = (req,res) => {
	res.sendStatus(200);
	var response = {} ;
	var listedarray = [];
	var pfData = [];
  ...
  if(listedarray.includes(ip.toString()) === true){
		 ...
		 let UpdateSql = "UPDATE user_video_plan SET ";
		 UpdateSql += " payment_status = 'done' ";
		 UpdateSql += " WHERE invoice_id = "+payment.order_number+"";
	...
	var mailOption = {
					subject: 'console log of notify Payment video plan',
					body: ip,
...
module.exports = videoPlanNotifyPayFast;
