...
const requestController = {
  ...
  create: async (req, res) => {
    ...
    } else if (therapy_type === "registered councillor" || therapy_type === "registered counsellor") {
	...
	var utcCutoff = moment.utc(schedule_date, 'YYYY-MM-DD HH:mm:ss');
    ...
    try {
      ...
      data.total_amount = price - referral_discount_amount;
      data.apointment_date_time = schedule_date_UTC;
      // check promocode
      if (promo_code) {
        ...
        if(data.discount_promo_code>0){
			data.price = data.price - promores.discount_amount;
		}
      }
      ...
      if (data.payment_status == "done") {
        ...
        var APPOINTMENTDATE = moment(schedule_date).format("DD MMM YYYY hh:mm A");
        ...
        if (requestsData[0].therapist_device_type != "web") {
          ...
          notificationData.status = requestsData[0].status;
  ...
  edit: async (req, res) => {
    ...
    try {
      ...
      if (requestsData.therapist_device_type != "web") {
        ...
        notificationData.status = requestsData.status;
        notificationData.type = "therapist_job_detail";
    ...
    try {
      ...
      if (result.users_master) {
        ...
        if (result.users_master.mobile_number) {
          ...
          // removed ) on landing 
          result.users_master.mobile_number = result.users_master.mobile_number.replace(/^0+/, '');
          result.users_master.mobile_number =
            "+" +
  ...
  /**
   * confirm/cancel duplicate request
   */
    ...
    try {
      ...
      if (status === "confirm") {
        ...
        let reponse_data = {
          ...
          notify_url: constants.PAYFAST_NOTIFY_URL_DUPLICATE,
          is_redirect: is_redirect,
        };
        response.result = reponse_data;
      }
      // send emil to admin on confirm/cancel booking
      if (process.env.MAIL_SEND.toString() === "true" && is_redirect=="no") {
      ...
      //Send email to therapist on confirm/cancel booking
      if (process.env.MAIL_SEND.toString() === "true" && is_redirect=="no") {
        var user_email_data = {
          id: 16,
        };
      ...
      // send notification
      if (result.users_master.device_type != "web" && is_redirect=="no") {
        webIds.push(result.users_master.notification_key);
        notificationData.id = result.id;
        notificationData.title = title;
        notificationData.message = message;
      ...
      logger.log(
        "error",
        "try-catch: requestController.confirm cancel failed.",
        err
      );
      res.statusCode = 500;
      return res.json(response);
  ...
  confirmReorder: async (req, res) => {
    ...
    let promo_code = req.body.promo_code ? req.body.promo_code : "";
    let total_amount = req.body.total_amount
      ? req.body.total_amount
      : "";
    let discount_promo_code = req.body.discount_promo_code
      ? req.body.discount_promo_code : "";
    ...
    try {
      ...
      //data.status = "pending";
      // add request
      let insert_request_result = await services.updateReorderRequest(data);
      if (insert_request_result === "result_failed") {
        res.statusCode = 500;
        return res.json(response);
      }
      ...
      let reponse_data = {
        id: result.id,
        request_number: result.request_number,
        cancel_url: constants.PAYFAST_CANCEL_URL + "?id=" + result.request_number,
        return_url: constants.PAYFAST_RETURN_URL + "?id=" + result.request_number,
        notify_url: constants.PAYFAST_NOTIFY_URL_DUPLICATE,
        is_redirect: is_redirect,
      };
      response.statusCode = 200;
