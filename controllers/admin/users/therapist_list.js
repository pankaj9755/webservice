...
TherapistList = (req, res) => {
	...
	var query = "SELECT id, first_name, last_name, CONCAT(first_name, ' ', last_name) AS fullname, email, mobile_number, address, profile_image, status, therapy_profile_status FROM users_master WHERE user_type = 'therapist' AND deleted_at IS NULL";

	var que_cnt = "SELECT id, first_name, last_name, CONCAT(first_name, ' ', last_name) AS fullname, email, mobile_number, address, profile_image, status, therapy_profile_status FROM users_master WHERE user_type = 'therapist' AND deleted_at IS NULL";
		...
		if (resul.length > 0) {
			...
			.then(result_cnt => {
				...
				resul.forEach(function (element) {
					data.push({ id: element.id, first_name: element.first_name, last_name: element.last_name, email: UtilityHelper.decrypted(element.email), mobile_number: UtilityHelper.decrypted(element.mobile_number), address: element.address, profile_image: element.profile_image, status: element.status, therapy_profile_status: element.therapy_profile_status });
				});	// End forEach loop.
