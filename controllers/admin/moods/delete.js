...
var deleteMoods = (req, res) => {
  ...
  let select_query =
    "SELECT id FROM moods WHERE id = :id AND deleted_at IS NULL";
  dbConnection
    .query(select_query, {
      replacements: { id: id },
    ...
    .then((result) => {
      ...
      let soft_delete_query =
        "UPDATE moods SET deleted_at = :deleted_at WHERE id = :id";
      dbConnection
        ...
        .then((result) => {
          ...
          response.message = "Moods deleted successfully.";
          return res.send(response);
        })
        .catch((err) => {
          console.error("Error in moods.delete.js: " + err);
    ...
    .catch((err) => {
      console.error("Error in assessments.select.js: " + err);
      res.statusCode = constants.SOMETHING_WENT_WRONG_STATUS_CODE;
      response.message = constants.SOMETHING_WENT_WRONG;
      return res.send(response);
    });
