  ...
  dbConnection
    ...
    .then((result) => {
      ...
      let soft_delete_query =
        "UPDATE group_code SET deleted_at = :deleted_at WHERE id = :id";
      dbConnection
        ...
        .then((result) => {
          ...
          response.message = "Group code deleted successfully.";
          return res.send(response);
        })
        .catch((err) => {
          console.error("Error in group_code.delete.js: " + err);
    ...
    .catch((err) => {
      console.error("Error in library.select.js: " + err);
      res.statusCode = constants.SOMETHING_WENT_WRONG_STATUS_CODE;
      response.message = constants.SOMETHING_WENT_WRONG;
      return res.send(response);
    });
