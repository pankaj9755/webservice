...
var findAll = (req, res) => {
  ...
  let query =
    "SELECT id, title, description, type, file, status, created_at FROM exercises WHERE deleted_at IS NULL";
  let que_cnt =
    "SELECT COUNT(*) AS total FROM exercises WHERE deleted_at IS NULL";
  if (status != "") {
  ...
  Promise.all(promises)
    .then((result) => {
      ...
      if (data.count > 0) {
        ...
        response.message = "Exercise List";
      } else {
    ...
    .catch((err) => {
      console.error("Error in exercises list.js: ", err);
