...
var findOne = (req, res) => {
  ...
  let query =
    "SELECT id, title, description, image, status FROM assessments WHERE id = :id AND deleted_at IS NULL";
  dbConnection
    .query(query, {
      replacements: { id: id },
    ...
    .then((result) => {
      ...
      response.message = "Assessments detail";
      response.result = result;
      return res.send(response);
    })
    .catch((err) => {
      console.error("Error in assessments.detail.js: " + err);
