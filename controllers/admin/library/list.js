...
var findAll = (req, res) => {
  ...
  let query =
    "SELECT id, topic, title, description, type, file, status, created_at FROM libraries WHERE deleted_at IS NULL";
  let que_cnt =
    "SELECT COUNT(*) AS total FROM libraries WHERE deleted_at IS NULL";
  if (status != "") {
  ...
  Promise.all(promises)
    ...
    .catch((err) => {
      console.error("Error in library list.js: ", err);
