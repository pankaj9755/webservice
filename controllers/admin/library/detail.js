...
var findOne = (req, res) => {
  ...
  let query =
    "SELECT id, topic, title, description, type, file, thum_img, status FROM libraries WHERE id = :id AND deleted_at IS NULL";
  dbConnection
    .query(query, {
      replacements: { id: id },
    ...
    .then((result) => {
      ...
      response.message = "library detail";
      response.result = result;
      return res.send(response);
    })
    .catch((err) => {
      console.error("Error in library.detail.js: " + err);
